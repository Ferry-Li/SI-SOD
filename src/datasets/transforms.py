...
class Scale(object):
    ...
    def __call__(self, img, label):
        '''
        :param img: RGB image
        :param label: semantic label image, maybe list
        ...
        if type(label) == list:
            for idx, _ in enumerate(label):
                label[idx] = cv2.resize(label[idx], (self.w, self.h), interpolation=cv2.INTER_NEAREST)
        else:
...
class Resize(object):
    ...
    def __call__(self, image, label):
        ...
        if type(label) == list:
            for idx, _ in enumerate(label):
                label[idx] = cv2.resize(label[idx], size, interpolation=cv2.INTER_NEAREST)
        else:
...
class RandomCropResize(object):
    ...
    def __call__(self, img, label):
        if random.random() < 0.5:
            ...
            # apply same transform to mask and weight
            if type(label) == list:
                label_crop = list()
                for item in enumerate(label):
                    # print(item.shape)
                    new_item = item[y1:h-y1, x1:w-x1]
                    # print(f'h-y1:{h-y1}')
                    label_crop.append(cv2.resize(new_item, (w, h), interpolation=cv2.INTER_NEAREST))
    ...
    def __call__(self, image, label):
        if random.random() < 0.5:
            ...
            if x1 == 0:
                ...
                if type(label) == list:
                    for idx, _ in enumerate(label):
                        label[idx] = cv2.flip(label[idx], 0)
                else:
                ...
                if type(label) == list:
                    for idx, _ in enumerate(label):
                        label[idx] = cv2.flip(label[idx], 1)
...
class GradientNormalize(object):
    """
    Given mean: (B, G, R) and std: (B, G, R),
    will normalize each channel of the torch.*Tensor, i.e.
    channel = (channel - mean) / std
...
class ToTensor(object):
    ...
    def __call__(self, image, label):
        if self.scale != 1:
            ...
            if type(label) == list:
                for idx, _ in enumerate(label):
                    label[idx] = cv2.resize(label[idx], (int(w/self.scale), int(h/self.scale)), \
        ...
        if type(label) == list:
            label_tensor = list()
            for idx, _ in enumerate(label):
                label_tensor.append(torch.FloatTensor(np.array(label[idx], dtype=float)))
            return [image_tensor.float(), label_tensor]
        else:
            # label_tensor =  torch.LongTensor(np.array(label, dtype=int))
            label_tensor =  torch.FloatTensor(np.array(label, dtype=float))
