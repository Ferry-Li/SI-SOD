...
from .utils import ConvBNReLU, ReceptiveConv
from .vgg import vgg16
from .resnet import resnet50, resnet101, resnet152, Bottleneck
...
class EDN(nn.Module):
    def __init__(self, config, is_train=True, log=None,
        ...
        self.freeze_s1 = config["freeze_s1"]
        self.is_train = is_train
        self.log = log
        self.log.info(f"arch: {self.arch}")
        self.log.info(f"freeze_s1: {self.freeze_s1}")

        if self.arch == 'vgg16':
            enc_channels=[64, 128, 256, 512, 512, 256, 256]#, 256, 256]
        elif 'resnet50' in self.arch:
        ...
        elif 'p2t_small' in self.arch:
            enc_channels=[64, 128, 320, 512, 256, 256]
            dec_channels=[32, 64, 128, 256, 128, 128]

        if 'resnet' in self.arch:
            pretrained = config["resnet_path"]
        elif 'mobilenetv2' in self.arch:
            pretrained = config["mobilenetv2_path"]
        elif 'p2t_small' in self.arch:
            pretrained = config["p2t_path"]
        self.backbone = eval(self.arch)(pretrained)
        self.log.info(f"load backbone from {pretrained}")

        use_dwconv = 'mobilenet' in self.arch
        
        if 'vgg' in self.arch or 'p2t' in self.arch:
        ...
        self._freeze_backbone(freeze_s1=self.freeze_s1)
    ...
    def forward(self, input):
        ...
        if self.is_train:
            ...
            return torch.sigmoid(saliency_maps[0].squeeze(1))
