...
class ICON(torch.nn.Module):
    def __init__(self, cfg, is_train=True, log=None):
        ...
        if self.model_name == 'ICON-S':
            ...
            pretrained_dict = torch.load(cfg["swin_path"])["model"]
            ...
            pretrained_dict = torch.load(cfg["pvt_path"])
            pretrained_dict = {k: v for k, v in pretrained_dict.items() if k in self.encoder.state_dict()}
            self.encoder.load_state_dict(pretrained_dict)
            ...
            pretrained_dict = torch.load(cfg["resnet_path"])
            encoder_dict = self.encoder.state_dict()
            pretrained_dict = {k: v for k, v in pretrained_dict.items() if k in encoder_dict}
            encoder_dict.update(pretrained_dict)
            self.log.info(f"load resnet from {cfg['resnet_path']}")
            ...
            pretrained_dict = torch.load(cfg["CycleMLP_path"])
    ...
    def forward(self, x, shape=None, name=None):
        ...
        pred1  = torch.sigmoid(F.interpolate(self.predtrans1(x1),   size=shape, mode='bilinear'))
        pred2  = torch.sigmoid(F.interpolate(self.predtrans2(x2),   size=shape, mode='bilinear')) 
        pred3  = torch.sigmoid(F.interpolate(self.predtrans3(x3),   size=shape, mode='bilinear'))
        pred4  = torch.sigmoid(F.interpolate(self.predtrans4(x4),   size=shape, mode='bilinear'))
        pose   = torch.sigmoid(F.interpolate(self.predtrans5(pose), size=shape, mode='bilinear'))

        if self.is_train:
            return torch.concat((pred1, pred2, pred3, pred4, pose), dim=1)
        else:
            return pred4.squeeze(1)
