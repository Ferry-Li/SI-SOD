...
class PoolNet(nn.Module):
    def __init__(self, base_model_cfg, base, convert_layers, deep_pool_layers, score_layers, cfg, is_train=True, log=None):
    ...
    def forward(self, x):
        ...
        return torch.sigmoid(merge.squeeze(1))

def build_PoolNet(cfg, is_train=True, log=None):
    base_model_cfg = cfg["backbone"]
    if base_model_cfg == 'vgg':
        return PoolNet(base_model_cfg, *extra_layer(base_model_cfg, vgg16_locate()), cfg, is_train=is_train, log=log)
    elif base_model_cfg == 'resnet':
        return PoolNet(base_model_cfg, *extra_layer(base_model_cfg, resnet50_locate()), cfg, is_train=is_train, log=log)
